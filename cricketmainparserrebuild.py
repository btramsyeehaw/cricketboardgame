...
over_length = 5
target_bowler_file = "target2022HundredBowlers.csv"
target_batter_file = "target2022HundredBatters.csv"
	...
	for i in range(len(overtypes)):
		...
		if len(cutballs)>0:
			...
			if x>108:
				x=108
			if x>0:
				dots = add_pre_to_six_rep(np.base_repr(int(sumtaken),6),"1") + " - " + add_pre_to_six_rep(np.base_repr(int(x+sumtaken-1),6),"1")
			else:
				dots = "-"
			sumtaken = sumtaken+x
			hitstaken.append(0)
			...
			def addrating(hittype):
				...
				if len(cutballs)-sumt>0:
					...
					if variable>0:
						cardval = add_pre_to_six_rep(np.base_repr(int(sumtaken),6),"1") + " - " + add_pre_to_six_rep(np.base_repr(int(variable+sumtaken-1),6),"1")
...
for testbatter in list(batter_file["striker"].to_list()):
	...
	wickets = testrob.count('W')
	batting_sr = round(100*get_run_count(testrob)/ballsfaced,2) if wickets>0 else '-' 
	...
	for i in range(len(overtypes)):
		...
		if len(cutballs)>0:
			...
			if x>108:
				x=108
			if x>0:
				dots = add_pre_to_six_rep(np.base_repr(int(sumtaken),6),"4") + " - " + add_pre_to_six_rep(np.base_repr(int(x+sumtaken-1),6),"4")
			else:
				dots = "-"
			sumtaken = sumtaken+x
			hitstaken.append(0)
